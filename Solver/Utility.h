...
#ifndef SMART_LCG_OIL_DELIVERY_UTILITY_H
#define SMART_LCG_OIL_DELIVERY_UTILITY_H
...
protected:
    ...
    void allocate(IndexType length) {
        // TODO[lcg][2]: length > (1 << 32)?
        arr = new T[static_cast<size_t>(length)];
        len = length;
    }

    void copyData(T *data) {
        // TODO[lcg][1]: what if data is shorter than arr?
        // OPTIMIZE[lcg][8]: use memcpy() if all callers are POD type.
...
public:
    ...
    // TODO[lcg][8]: use different names for the arguments.
...
#endif // SMART_LCG_OIL_DELIVERY_UTILITY_H
